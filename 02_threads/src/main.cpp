...
public:
  ...
  void operator()(int laps) {
    ...
    for (int i = 0; i < laps; i++) {
      ...
      ges += rundenzeit;
      buf << "Klasse Car: Runde " << round << " Rundenzeit: " << setprecision(3)
          << rundenzeit << "s" << endl;
      output = buf.str();
      buf.str("");
      cout << output << flush;
  ...
  float get_total_time() { return ges; }
};
...
void mini_one(float &n, int laps) {
  ...
  for (int i = 0; i < laps; i++) {
    ...
    float rundenzeit = dis(gen);
    n += rundenzeit;
    buf << "Mini One: Runde " << round << " Rundenzeit: " << setprecision(3)
        << rundenzeit << "s" << endl;
    output = buf.str();
    buf.str("");
    cout << output << flush;
...
int main(int argc, char *argv[]) {
  ...
  if (n < audi_a1.get_total_time()) {
    ...
    cout << "Der Sieger ist Audi mit      : " << audi_a1.get_total_time()
         << endl;
